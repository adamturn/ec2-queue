...
def connect_postgres(props_path):
    ...
    print("Parsed keys:", ", ".join(props.keys()))

    print("Matching parsed keys to db config requirements...")
    reqs = {
        "dbhost": r"(?i)host",
        "dbport": r"(?i)port",
    ...
    for prop in props:
        ...
        continue
    if cfgkeys:
        raise ValueError("No match for required db config key: " + ", ".join(cfgkeys))
    else:
